...
export class StaticAPI {
    ...
    public async checkStaticProblemSetIdAvailable(setId: string) {
        const response: Response = await fetch(`${StaticAPI.API_URL}/check_set_id_available/${setId}`, {method: "GET"});
    ...
    public async createProblem(setId: string, content: TextOrImage) {
        ...
        const response = await fetch(`${StaticAPI.API_URL}/problems`, options);
    ...
    private static addAllTextOrImagesToFormData(key: string, contentList: Array<TextOrImage>, formData: FormData) {
        for (let i = 0; i < contentList.length; i++) {
            ...
            if (typeof item === "string") {
                ...
                formData.append(key + "Images[]", item.file);
            } else {
                continue;
            }
